  ...
  module Build
    class Script
      class Csharp < Script
          ...
          sh.fold('mono-install') do
            if mono_version_valid?
              ...
              if config[:mono] == '2.10.8'
                ...
                sh.cmd 'sudo apt-get update -qq', timing: true, assert: true
                sh.cmd "sudo apt-get install -qq mono-complete mono-vbnc fsharp nuget #{'referenceassemblies-pcl' if config[:mono] != '3.8.0'}", timing: true, assert: true
        ...
        def install
          sh.cmd "nuget restore #{config[:solution]}", retry: true if config[:solution] && config[:mono] != '2.10.8' && config[:mono] != '3.2.8'
        end
        ...
        def mono_version_valid?
          ...
          return false if MONO_VERSION_REGEXP.match(config[:mono])[1] == '2' && config[:mono] != '2.10.8'
